...
class FaissStore:
    def __init__(self, store_path: Path, rescore_multiplier: int = 20):
        if not store_path.is_dir():
            raise FileNotFoundError(f"FAISS store not found: {store_path}")
...
def hydrate_ids(conn: sqlite3.Connection, ids: List[str]) -> List[Dict[str, str]]:
    """Fetch labels for a list of IDs."""
    if not ids:
        return []
...
def calculate_accuracy(df_results: pd.DataFrame):
    ...
    print("\n--- Dense Search Only Evaluation Results ---")
...
def run_dense_evaluation():
    ...
    # Add Dense prediction columns to the original dataframe
    dense_predictions = []

    for _, row in tqdm(df_test.iterrows(), total=len(df_test), desc="Running Dense Baseline"):
        try:
            query_vector = np.array(embed_fn([row['source_term']]), dtype=np.float32)
            dense_ids = faiss_store.search(query_vector, k=MAX_K)[0]
            
            ontology_filter = allowed_ontologies_for(row['field'], row['organism'])
            if ontology_filter and dense_ids:
                qmarks = ",".join("?" for _ in dense_ids)
                cur = conn.cursor()
                cur.execute(
                    f"SELECT curie FROM {TABLE_TERMS} WHERE curie IN ({qmarks}) AND ontology_id IN ({','.join('?' for _ in ontology_filter)})",
                    dense_ids + ontology_filter
                )
                valid_ids = {r[0] for r in cur.fetchall()}
                dense_ids = [id_ for id_ in dense_ids if id_ in valid_ids]

            hydrated_hits = hydrate_ids(conn, dense_ids)
            dense_predictions.append(hydrated_hits)
        except Exception as e:
    ...
    # Add Dense prediction columns to original dataframe
    # Top-1 predictions
    df_test['dense_top1_predicted_id'] = [pred[0]['id'] if len(pred) >= 1 else None for pred in dense_predictions]
    df_test['dense_top1_predicted_label'] = [pred[0]['label'] if len(pred) >= 1 else None for pred in dense_predictions]
    
    # Top-3 predictions
    for i in range(3):
        df_test[f'dense_top3_rank{i+1}_id'] = [pred[i]['id'] if len(pred) > i else None for pred in dense_predictions]
        df_test[f'dense_top3_rank{i+1}_label'] = [pred[i]['label'] if len(pred) > i else None for pred in dense_predictions]
    
    # Top-5 predictions
    for i in range(5):
        df_test[f'dense_top5_rank{i+1}_id'] = [pred[i]['id'] if len(pred) > i else None for pred in dense_predictions]
        df_test[f'dense_top5_rank{i+1}_label'] = [pred[i]['label'] if len(pred) > i else None for pred in dense_predictions]
    
    # Save the enhanced CSV with original structure + Dense predictions
    output_path = "/Users/rajlq7/Downloads/Terms/BOND/evals/Dense-Only/dense_baseline_predictions.csv"
    df_test.to_csv(output_path, index=False)
    print(f"Dense baseline predictions appended and saved to {output_path}")
    print(f"Added {2 + 6 + 10} Dense prediction columns to original {len(df_test.columns) - 18} columns")
